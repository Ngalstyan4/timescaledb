...
transform_time_op_const_interval(OpExpr *op)
{
	...
	if (list_length(op->args) == 2 && IsA(lsecond(op->args), Const))
	{
		    ...
		    (left == DATEOID && right == INTERVALOID))
		{
			...
			{
				Expr *first =
				    sort_transform_expr((Expr *) linitial(op->args));
		    ...
		    (left == INT2OID && right == INT2OID))
		{
			...
			if (name[1] == '\0')
			{
				...
				case '*':
					...
					{
						Expr *nonconst = sort_transform_expr(
...
sort_transform_expr(Expr *orig_expr)
{
	...
	if (IsA(orig_expr, OpExpr))
	{
		...
		if (type_first == INT2OID || type_first == INT4OID ||
		    type_first == INT8OID)
		{
 ...
 *	expressions if any of the members of the original EC can be transformed for the
 *sort.
 */
...
sort_transform_optimization(PlannerInfo *root, RelOptInfo *rel)
{
	...
	foreach (lc_pathkey, root->query_pathkeys)
	{
		...
		if (transformed != NULL)
		{
			...
			transformed_query_pathkey =
			    lappend(transformed_query_pathkey, newpk);
		}
		else
		{
			transformed_query_pathkey =
			    lappend(transformed_query_pathkey, pk);
		}
	}
	...
	if (was_transformed)
	{
		...
		foreach (lc_plan, rel->pathlist)
		{
			...
			if (compare_pathkeys(path->pathkeys, transformed_query_pathkey) ==
			    PATHKEYS_EQUAL)
			{
