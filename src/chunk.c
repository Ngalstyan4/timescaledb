...
static int  chunk_scan_ctx_foreach_chunk(ChunkScanCtx *ctx, on_chunk_func on_chunk,
					 uint16 limit);
...
chunk_insert_relation(Relation rel, Chunk *chunk)
{
	...
	values[AttrNumberGetAttrOffset(Anum_chunk_schema_name)] =
	    NameGetDatum(&chunk->fd.schema_name);
	values[AttrNumberGetAttrOffset(Anum_chunk_table_name)] =
...
chunk_fill(Chunk *chunk, HeapTuple tuple)
{
	...
	chunk->table_id =
	    get_relname_relid(chunk->fd.table_name.data,
...
do_dimension_alignment(ChunkScanCtx *scanctx, Chunk *chunk)
{
	...
	for (i = 0; i < space->num_dimensions; i++)
	{
		 ...
		 */
		chunk_slice =
		    hypercube_get_slice_by_dimension_id(chunk->cube, dim->fd.id);
...
calculate_and_set_new_chunk_interval(Hypertable *ht, Point *p)
{
	...
	{
		elog(WARNING,
		     "adaptive chunking enabled on hypertable \"%s\" without an open "
		     "(time) dimension",
	...
	datum = OidFunctionCall3(
	    ht->chunk_sizing_func, dim->fd.id, coord, ht->fd.chunk_target_size);
...
chunk_add_constraints(Chunk *chunk)
{
	...
	num_added = chunk_constraints_add_dimension_constraints(
	    chunk->constraints, chunk->fd.id, chunk->cube);
	num_added += chunk_constraints_add_inheritable_constraints(
	    chunk->constraints, chunk->fd.id, chunk->hypertable_relid);
...
set_attoptions(Relation ht_rel, Oid chunk_oid)
{
	...
	for (attno = 1; attno <= natts; attno++)
	{
		 ...
		 * attribute_option)
		 */
		options = SysCacheGetAttr(
		    ATTNAME, tuple, Anum_pg_attribute_attoptions, &isnull);
		 ...
		 */
		options = SysCacheGetAttr(
		    ATTNAME, tuple, Anum_pg_attribute_attstattarget, &isnull);
		if (!isnull)
		{
...
create_toast_table(CreateStmt *stmt, Oid chunk_oid)
{
	...
	Datum	toast_options = transformRelOptions(
		   (Datum) 0, stmt->options, "toast", validnsps, true, false);
...
chunk_create_table(Chunk *chunk, Hypertable *ht)
{
	...
	CreateStmt    stmt = {
		   ...
		   .inhRelations = list_make1(makeRangeVar(
		       NameStr(ht->fd.schema_name), NameStr(ht->fd.table_name), 0)),
...
chunk_create_after_lock(Hypertable *ht, Point *p, const char *schema, const char *prefix)
{
	...
	chunk = chunk_create_stub(catalog_table_next_seq_id(catalog, CHUNK),
				  hs->num_dimensions);
	catalog_restore_user(&sec_ctx);
	...
	snprintf(
	    chunk->fd.table_name.data, NAMEDATALEN, "%s_%d_chunk", prefix, chunk->fd.id);
...
chunk_create_stub(int32 id, int16 num_constraints)
{
	...
	if (num_constraints > 0)
		chunk->constraints =
		    chunk_constraints_alloc(num_constraints, CurrentMemoryContext);
...
chunk_scan_ctx_init(ChunkScanCtx *ctx, Hyperspace *hs, Point *p)
{
	...
	ctx->htab = hash_create(
	    "chunk-scan-context", 20, &hctl, HASH_ELEM | HASH_CONTEXT | HASH_BLOBS);
	ctx->space = hs;
	ctx->point = p;
...
dimension_slice_and_chunk_constraint_join(ChunkScanCtx *scanctx, DimensionVec *vec)
{
	...
	for (i = 0; i < vec->num_slices; i++)
	{
		 ...
		 */
		chunk_constraint_scan_by_dimension_slice(
		    vec->slices[i], scanctx, CurrentMemoryContext);
	}
}
...
chunk_collision_scan(ChunkScanCtx *scanctx, Hypercube *cube)
{
	...
	for (i = 0; i < scanctx->space->num_dimensions; i++)
	{
		...
		vec = dimension_slice_collision_scan(
		    slice->fd.dimension_id, slice->fd.range_start, slice->fd.range_end);
...
chunk_scan_ctx_foreach_chunk(ChunkScanCtx *ctx, on_chunk_func on_chunk, uint16 limit)
{
	...
	for (entry = hash_seq_search(&status); entry != NULL;
...
{
	if (scanctx->space->num_dimensions !=
	    chunk->constraints->num_dimension_constraints)
		return false;
...
chunk_find(Hyperspace *hs, Point *p)
{
	...
	if (NULL != chunk)
	{
		 ...
		 */
		chunk->constraints = chunk_constraint_scan_by_chunk_id(
		    chunk->fd.id, hs->num_dimensions, CurrentMemoryContext);
	}
...
static int
chunk_scan_internal(int indexid, ScanKeyData scankey[], int nkeys,
		    tuple_found_func tuple_found, void *data, int limit,
		    ScanDirection scandir, LOCKMODE lockmode, MemoryContext mctx)
{
	Catalog *  catalog = catalog_get();
	ScannerCtx ctx = {
		.table = catalog->tables[CHUNK].id,
...
chunk_get_window(int32 dimension_id, int64 point, int count, MemoryContext mctx)
{
	...
	dimvec = dimension_slice_scan_by_dimension_before_point(
	    dimension_id, point, count, BackwardScanDirection, mctx);
	...
	for (i = 0; i < dimvec->num_slices; i++)
	{
		...
		for (j = 0; j < ccs->num_constraints; j++)
		{
			...
			chunk->constraints =
			    chunk_constraint_scan_by_chunk_id(chunk->fd.id, 1, mctx);
			chunk->cube =
			    hypercube_from_constraints(chunk->constraints, mctx);
		...
		MemoryContext mctx, bool fail_if_not_found)
{
	...
	case 1:
		...
		{
			chunk->constraints = chunk_constraint_scan_by_chunk_id(
			    chunk->fd.id, num_constraints, mctx);
			chunk->cube =
			    hypercube_from_constraints(chunk->constraints, mctx);
		}
		break;
...
Chunk *
chunk_get_by_name_with_memory_context(const char *schema_name, const char *table_name,
...
chunk_tuple_delete(TupleInfo *ti, void *data)
{
	...
	for (i = 0; i < ccs->num_constraints; i++)
	{
		...
		if (is_dimension_constraint(cc) &&
		    chunk_constraint_scan_by_dimension_slice_id(
			cc->fd.dimension_slice_id, NULL, CurrentMemoryContext) == 0)
...
chunk_delete_by_relid(Oid relid)
{
	...
	return chunk_delete_by_name(get_namespace_name(get_rel_namespace(relid)),
				    get_rel_name(relid));
}
...
chunk_recreate_all_constraints_for_dimension(Hyperspace *hs, int32 dimension_id)
{
	...
	for (i = 0; i < slices->num_slices; i++)
		chunk_constraint_scan_by_dimension_slice(
		    slices->slices[i], &chunkctx, CurrentMemoryContext);
...
chunk_update_form(FormData_chunk *form)
{
	...
	ScanKeyInit(
	    &scankey[0], Anum_chunk_idx_id, BTEqualStrategyNumber, F_INT4EQ, form->id);
