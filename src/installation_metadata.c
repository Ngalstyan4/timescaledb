...
#define TYPE_ERROR(inout, typeid)                                                        \
	elog(ERROR,                                                                      \
	     "ts::installation_metadata: no %s function for type %u",                    \
	     inout,                                                                      \
	     typeid);
...
installation_metadata_tuple_get_value(TupleInfo *ti, void *data)
{
	...
	dv->value = heap_getattr(
	    ti->tuple, Anum_installation_metadata_value, ti->desc, &dv->isnull);
					 ...
					 bool *isnull, LOCKMODE lockmode)
{
	...
	ScannerCtx scanctx = {
		...
		.index = CATALOG_INDEX(
		    catalog, INSTALLATION_METADATA, INSTALLATION_METADATA_PKEY_IDX),
		.nkeys = 1,
		.scankey = scankey,
...
Datum
installation_metadata_get_value(Datum metadata_key, Oid key_type, Oid value_type,
				bool *isnull)
{
	return installation_metadata_get_value_internal(
	    metadata_key, key_type, value_type, isnull, AccessShareLock);
}
 ...
 */
Datum
installation_metadata_insert(Datum metadata_key, Oid key_type, Datum metadata_value,
			     Oid value_type)
{
	...
	existing_value = installation_metadata_get_value_internal(
	    metadata_key, key_type, value_type, &isnull, ShareRowExclusiveLock);
