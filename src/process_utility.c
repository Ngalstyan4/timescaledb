...
{
	return foreach_chunk_relid(
	    RangeVarGetRelid(rv, NoLock, true), process_chunk, arg);
}
		...
		if (OidIsValid(relid))
		{
			...
			if (NULL != ht)
			{
				if (list_length(stmt->objects) != 1)
					elog(ERROR,
					     "cannot drop a hypertable along with other "
					     "objects");
		...
		ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("tablespace \"%s\" is still attached to %d hypertables",
				stmt->tablespacename,
				count),
			 errhint("Detach the tablespace from all hypertables before "
				 "removing it.")));
}
...
process_reindex(Node *parsetree)
{
	...
	case REINDEX_OBJECT_INDEX:
		...
		if (NULL != ht)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("reindexing of a specific index on a hypertable "
					"is unsupported"),
				 errhint("As a workaround, it is possible to run REINDEX "
					 "TABLE to reindex all "
					 "indexes on a hypertable, including all indexes "
					 "on chunks.")));
		}
		break;
	default:
...
process_rename_column(Cache *hcache, Oid relid, RenameStmt *stmt)
{
	...
	if (NULL == ht)
	{
		...
		if (NULL != chunk)
			ereport(
			    ...
			    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			     errmsg(
				 "cannot rename column \"%s\" of hypertable chunk \"%s\"",
				 stmt->subname,
				 get_rel_name(relid)),
	...
	dim = hyperspace_get_dimension_by_name(
	    ht->space, DIMENSION_TYPE_ANY, stmt->subname);
		...
		if (strncmp(stmt->subname, timescaledb_schema_names[i], NAMEDATALEN) == 0)
		{
			ereport(ERROR,
				(errcode(ERRCODE_TS_OPERATION_NOT_SUPPORTED),
				 errmsg("cannot rename schemas used by the TimescaleDB "
					"extension")));
			return;
		}
	}
...
rename_hypertable_constraint(Hypertable *ht, Oid chunk_relid, void *arg)
{
	...
	chunk_constraint_rename_hypertable_constraint(
	    chunk->fd.id, stmt->subname, stmt->newname);
}
...
alter_hypertable_constraint(Hypertable *ht, Oid chunk_relid, void *arg)
{
	...
	cmd_constraint->conname = chunk_constraint_get_name_from_hypertable_constraint(
	    chunk_relid, hypertable_constraint_name);
...
validate_hypertable_constraint(Hypertable *ht, Oid chunk_relid, void *arg)
{
	...
	chunk_cmd->name =
	    chunk_constraint_get_name_from_hypertable_constraint(chunk_relid, cmd->name);
...
process_altertable_drop_not_null(Hypertable *ht, AlterTableCmd *cmd)
{
	...
	for (i = 0; i < ht->space->num_dimensions; i++)
	{
		...
		if (IS_OPEN_DIMENSION(dim) &&
			...
			ereport(ERROR,
				(errcode(ERRCODE_TS_OPERATION_NOT_SUPPORTED),
				 errmsg("cannot drop not-null constraint from a "
					"time-partitioned column")));
	}
}
		...
		if (namestrcmp(&dim->fd.column_name, cmd->name) == 0)
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
				 ...
				 errdetail("cannot drop column that is a hypertable "
					   "partitioning (space or time) dimension")));
	}
}
...
process_index_chunk(Hypertable *ht, Oid chunk_relid, void *arg)
{
	...
	chunk_index_create_from_stmt(
	    stmt, chunk->fd.id, chunk_relid, ht->fd.id, info->obj.objectId);
}
...
process_cluster_start(Node *parsetree, ProcessUtilityContext context)
{
	...
	if (NULL != ht)
	{
		...
		if (NULL == stmt->indexname)
			...
			index_relid = get_relname_relid(
			    stmt->indexname, get_rel_namespace(ht->main_table_relid));
		...
		mcxt = AllocSetContextCreate(
		    PortalContext, "Hypertable cluster", ALLOCSET_DEFAULT_SIZES);
...
process_alter_column_type_end(Hypertable *ht, AlterTableCmd *cmd)
{
	...
	Dimension *dim =
	    hyperspace_get_dimension_by_name(ht->space, DIMENSION_TYPE_ANY, cmd->name);
...
{
	Oid index_relid = get_relname_relid(
	    cmd->name, get_namespace_oid(NameStr(ht->fd.schema_name), false));
...
process_altertable_set_tablespace_end(Hypertable *ht, AlterTableCmd *cmd)
{
	...
	if (tspcs->num_tablespaces > 1)
		ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("cannot set new tablespace when multiple tablespaces are "
				"attached to hypertable \"%s\"",
				get_rel_name(ht->main_table_relid)),
	...
	{
		Assert(
		    hypertable_has_tablespace(ht, tspcs->tablespaces[0].tablespace_oid));
		tablespace_delete(ht->fd.id,
...
process_altertable_start_table(Node *parsetree)
{
	...
	foreach (lc, stmt->cmds)
	{
		...
		case AT_AddColumnRecurse:
		{
			...
			if (NULL == ht)
				foreach (constraint_lc, col->constraints)
					verify_constraint_plaintable(
					    stmt->relation, lfirst(constraint_lc));
			else
				foreach (constraint_lc, col->constraints)
					verify_constraint_hypertable(
					    ht, lfirst(constraint_lc));
			break;
		}
		case AT_DropColumn:
		...
		case AT_AddConstraintRecurse:
			...
			if (NULL == ht)
				verify_constraint_plaintable(stmt->relation,
							     (Constraint *) cmd->def);
			else
				verify_constraint_hypertable(ht, cmd->def);
			break;
...
process_altertable_end_simple_cmd(Hypertable *ht, CollectedCommand *cmd)
{
	...
	process_altertable_end_subcmd(
	    ht, linitial(stmt->cmds), &cmd->d.simple.secondaryObject);
}
...
process_ddl_command_start(ProcessUtilityArgs *args)
{
	...
	case T_GrantRoleStmt:
		...
		break;
	case T_CopyStmt:
		handled = process_copy(
		    args->parsetree, args->query_string, args->completion_tag);
		break;
	case T_VacuumStmt:
...
process_drop_constraint_on_chunk(Hypertable *ht, Oid chunk_relid, void *arg)
{
	...
	chunk_constraint_delete_by_hypertable_constraint_name(
	    chunk->fd.id, hypertable_constraint_name, true, true);
}
...
process_drop_table_constraint(EventTriggerDropObject *obj)
{
	...
	if (ht != NULL)
	{
		...
		foreach_chunk(
		    ht, process_drop_constraint_on_chunk, constraint->constraint_name);
	...
	{
		Chunk *chunk =
		    chunk_get_by_name(constraint->schema, constraint->table, 0, false);

		if (NULL != chunk)
		{
			chunk_constraint_delete_by_constraint_name(
			    chunk->fd.id, constraint->constraint_name, true, false);
		}
	}
}
...
process_drop_schema(EventTriggerDropObject *obj)
{
		...
		ereport(
		    ...
		    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		     errmsg("cannot drop the internal schema for extension \"%s\"",
			    EXTENSION_NAME),
		     errhint(
			 "Use DROP EXTENSION to remove the extension and the schema.")));
	...
	if (count > 0)
		ereport(
		    NOTICE,
		    (errmsg(
			"the chunk storage schema changed to \"%s\" for %d hypertable%c",
...
process_drop_trigger(EventTriggerDropObject *obj)
{
	...
	if (ht != NULL)
	{
		...
		foreach_chunk(
		    ht, process_drop_trigger_on_chunk, trigger_event->trigger_name);
	}
}
 ...
 */
Datum ts_timescaledb_process_ddl_event(PG_FUNCTION_ARGS)
{
