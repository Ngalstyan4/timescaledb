		...
		if (!index_has_attribute(indexelems, NameStr(dim->fd.column_name)))
			ereport(ERROR,
				(errcode(ERRCODE_TS_BAD_HYPERTABLE_INDEX_DEFINITION),
				 errmsg("cannot create a unique index without the column "
					"\"%s\" (used in partitioning)",
...
build_indexcolumn_list(Relation idxrel)
{
	...
	for (i = 0; i < idxrel->rd_att->natts; i++)
		columns = lappend(columns,
				  makeString(NameStr(idxrel->rd_att->attrs[i]->attname)));
...
create_default_index(Hypertable *ht, List *indexelems)
{
	IndexStmt stmt = {
		...
		.tableSpace =
		    get_tablespace_name(get_rel_tablespace(ht->main_table_relid)),
...
static void
create_default_indexes(Hypertable *ht, Dimension *time_dim, Dimension *space_dim,
					      ...
					      bool verify)
{
	...
	Dimension *space_dim =
	    hyperspace_get_dimension(ht->space, DIMENSION_TYPE_CLOSED, 0);
	...
	foreach (lc, indexlist)
	{
		...
		if (verify &&
		    (idxrel->rd_index->indisunique || idxrel->rd_index->indisexclusion))
			indexing_verify_columns(ht->space,
						build_indexcolumn_list(idxrel));
		...
		if (create_default && NULL != time_dim)
		{
			...
			case 1:
				/* ("time") index */
				if (namestrcmp(&attrs[0]->attname,
					       NameStr(time_dim->fd.column_name)) == 0)
					has_time_idx = true;
				break;
			case 2:
				/* ("space", "time") index */
				if (space_dim != NULL &&
				    namestrcmp(&attrs[0]->attname,
					       NameStr(space_dim->fd.column_name)) == 0 &&
				    namestrcmp(&attrs[1]->attname,
					       NameStr(time_dim->fd.column_name)) == 0)
	...
	if (create_default)
		create_default_indexes(
		    ht, time_dim, space_dim, has_time_idx, has_time_space_idx);
