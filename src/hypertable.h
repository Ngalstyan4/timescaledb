...
extern int hypertable_scan_with_memory_context(const char *schema, const char *table,
					       tuple_found_func tuple_found, void *data,
					       LOCKMODE lockmode, bool tuplock,
					       MemoryContext mctx);
extern int hypertable_scan_relid(Oid table_relid, tuple_found_func tuple_found,
				 void *data, LOCKMODE lockmode, bool tuplock);
...
extern Tablespace *hypertable_get_tablespace_at_offset_from(Hypertable *ht,
							    Oid		tablespace_oid,
							    int16       offset);
...
#define hypertable_scan(schema, table, tuple_found, data, lockmode, tuplock)             \
	hypertable_scan_with_memory_context(                                             \
	    schema, table, tuple_found, data, lockmode, tuplock, CurrentMemoryContext)
