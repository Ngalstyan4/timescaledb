...
#define IS_VALID_OPEN_DIM_TYPE(type)                                                     \
	(IS_INTEGER_TYPE(type) || IS_TIMESTAMP_TYPE(type) ||                             \
	 type_is_int8_binary_compatible(type))
...
extern Hyperspace *    dimension_scan(int32 hypertable_id, Oid main_table_relid,
				      int16 num_dimension, MemoryContext mctx);
extern DimensionSlice *dimension_calculate_default_slice(Dimension *dim, int64 value);
extern Point *	 hyperspace_calculate_point(Hyperspace *h, HeapTuple tuple,
						  TupleDesc tupdesc);
...
extern Dimension *hyperspace_get_dimension_by_name(Hyperspace *hs, DimensionType type,
						   const char *name);
