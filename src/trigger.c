...
{
	Datum datum_def =
	    DirectFunctionCall1(pg_get_triggerdef, ObjectIdGetDatum(trigger_oid));
	...
	CreateTrigger(stmt, def, InvalidOid, InvalidOid, InvalidOid, InvalidOid, false);
...
trigger_create_all_on_chunk(Hypertable *ht, Chunk *chunk)
{
	...
	if (!HeapTupleIsValid(tuple))
		elog(ERROR,
		     "cache lookup failed for relation ID %u",
		     ht->main_table_relid);
	...
	if (saved_uid != form->relowner)
		SetUserIdAndSecContext(form->relowner,
				       sec_ctx | SECURITY_LOCAL_USERID_CHANGE);
