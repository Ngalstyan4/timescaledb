...
extern void  chunk_index_create_all(int32 hypertable_id, Oid hypertable_relid,
				    int32 chunk_id, Oid chunkrelid);
extern Oid   chunk_index_create_from_stmt(IndexStmt *stmt, int32 chunk_id, Oid chunkrelid,
...
extern void  chunk_index_delete_by_name(const char *schema, const char *index_name,
					bool drop_index);
extern int   chunk_index_rename(Chunk *chunk, Oid chunk_indexrelid, const char *newname);
extern int   chunk_index_rename_parent(Hypertable *ht, Oid hypertable_indexrelid,
				       const char *newname);
extern int   chunk_index_set_tablespace(Hypertable *ht, Oid hypertable_indexrelid,
					const char *tablespace);
extern void  chunk_index_create_from_constraint(int32 hypertable_id,
						Oid hypertable_constaint, int32 chunk_id,
						Oid chunk_constraint);
extern List *chunk_index_get_mappings(Hypertable *ht, Oid hypertable_indexrelid);
extern ChunkIndexMapping *
 ...
 *
 * The process is split up into phase 1 and 2 because phase 1 does not lock reads and is
 *slow but phase 2 takes read locks but is quick. So if processing multiple tables you
 *first want to process all tables in phase 1 to completion and then run phase 2 on all
 *tables.
 *
