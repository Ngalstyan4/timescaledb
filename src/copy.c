...
copy_chunk_state_create(Hypertable *ht, Relation rel, CopyFromFunc from_func,
			void *fromctx)
{
...
timescaledb_CopyFrom(CopyChunkState *ccstate, List *range_table, Hypertable *ht)
{
	...
	for (;;)
	{
		...
		if (!ccstate->next_copy_from(
			ccstate, econtext, values, nulls, &loaded_oid))
			break;
		...
		if (!skip_tuple)
		{
...

			{
				...
				ExecARInsertTriggersCompat(
				    estate, resultRelInfo, tuple, recheckIndexes);
...
void
timescaledb_DoCopy(const CopyStmt *stmt, const char *queryString, uint64 *processed,
		   Hypertable *ht)
{
	CopyChunkState *ccstate;
	CopyState       cstate;
		...
		if (stmt->is_program)
			ereport(
			    ...
			    (errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
			     errmsg("must be superuser to COPY to or from an external "
				    "program"),
	...
	cstate = BeginCopyFrom(
	    rel, stmt->filename, stmt->is_program, stmt->attlist, stmt->options);
...
timescaledb_move_from_table_to_chunks(Hypertable *ht, LOCKMODE lockmode)
{
	...
	ccstate =
	    copy_chunk_state_create(ht, rel, next_copy_from_table_to_chunks, scandesc);
