...
bgw_policy_drop_chunks_tuple_found(TupleInfo *ti, void *const data)
{
	...
	*policy = MemoryContextAllocZero(ti->mctx, sizeof(BgwPolicyDropChunks));
	(*policy)->fd.job_id = DatumGetInt32(
		heap_getattr(ti->tuple, Anum_bgw_policy_drop_chunks_job_id, ti->desc, &isnull));
	Assert(!isnull);
	(*policy)->fd.hypertable_id = DatumGetInt32(
		heap_getattr(ti->tuple, Anum_bgw_policy_drop_chunks_hypertable_id, ti->desc, &isnull));
	Assert(!isnull);
	(*policy)->fd.interval_support = DatumGetBool(
		heap_getattr(ti->tuple, Anum_bgw_policy_drop_chunks_interval_support, ti->desc, &isnull));
	Assert(!isnull);
	if ((*policy)->fd.interval_support)
	{
		(*policy)->fd.older_than_interval =
			*DatumGetIntervalP(heap_getattr(ti->tuple,
	...
	{
		(*policy)->fd.older_than_integer =
			DatumGetInt64(heap_getattr(ti->tuple,
									   Anum_bgw_policy_drop_chunks_older_than_integer,
									   ti->desc,
									   &isnull));
		Assert(!isnull);
	}
	(*policy)->fd.cascade = DatumGetBool(
		heap_getattr(ti->tuple, Anum_bgw_policy_drop_chunks_cascade, ti->desc, &isnull));
	Assert(!isnull);
	(*policy)->fd.cascade_to_materializations =
		DatumGetBool(heap_getattr(ti->tuple,
