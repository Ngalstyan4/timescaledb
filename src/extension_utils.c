...
extension_version(void)
{
	...
	{
		result = heap_getattr(
		    tuple, Anum_pg_extension_extversion, RelationGetDescr(rel), &is_null);
...
extension_load_without_preload()
{
	...
	char *allow_install_without_preload = GetConfigOptionByName(
	    "timescaledb.allow_install_without_preload", NULL, true);
	    ...
	    strcmp(allow_install_without_preload, "on") != 0)
	{
		...
		{
			char *config_file =
			    GetConfigOptionByName("config_file", NULL, false);

			ereport(
			    FATAL,
			    (errmsg("extension \"%s\" must be preloaded", EXTENSION_NAME),
			     errhint(
				 "Please preload the timescaledb library via "
				 "shared_preload_libraries.\n\n"
				 "This can be done by editing the config file at: %1$s\n"
				 "and adding 'timescaledb' to the list in the "
				 "shared_preload_libraries config.\n"
				 "	# Modify postgresql.conf:\n	"
				 "shared_preload_libraries = 'timescaledb'\n\n"
				 "Another way to do this, if not preloading other "
				 "libraries, is with the command:\n"
				 "	echo \"shared_preload_libraries = "
				 "'timescaledb'\" >> %1$s \n\n"
				 "(Will require a database restart.)\n\n"
				 "If you REALLY know what you are doing and would like "
				 "to load the library without preloading, you can "
		...
		else
		{
			ereport(
			    ...
			    (errmsg("extension \"%s\" must be preloaded", EXTENSION_NAME),
				 ...
				 "libraries, is with the command:\n"
				 "	echo \"shared_preload_libraries = "
				 "'timescaledb'\" >> /path/to/config/file \n\n"
				 "(Will require a database restart.)\n\n"
				 "If you REALLY know what you are doing and would like "
				 "to load the library without preloading, you can "
				 "disable this check with: \n"
