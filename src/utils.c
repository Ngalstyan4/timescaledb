 ...
 */
Datum ts_pg_timestamp_to_microseconds(PG_FUNCTION_ARGS)
{
	...
	if (1)
	{
		...
		microseconds =
		    (seconds * USECS_PER_SEC) + ((timestamp - seconds) * USECS_PER_SEC);
	}
#endif
 ...
 */
Datum ts_pg_microseconds_to_timestamp(PG_FUNCTION_ARGS)
{
 ...
 */
Datum ts_pg_timestamp_to_unix_microseconds(PG_FUNCTION_ARGS)
{
	TimestampTz timestamp = PG_GETARG_TIMESTAMPTZ(0);
	int64       epoch_diff_microseconds =
	...
	if (1)
	{
		...
		microseconds = (seconds * USECS_PER_SEC) +
			       ((timestamp - seconds) * USECS_PER_SEC) +
 ...
 * Convert BIGINT microseconds relative the UNIX epoch to a Postgres TIMESTAMP.
 */
Datum ts_pg_unix_microseconds_to_timestamp(PG_FUNCTION_ARGS)
{
	int64       microseconds = PG_GETARG_INT64(0);
	TimestampTz timestamp;
	 ...
	 */
	if (microseconds <
	    ((int64) USECS_PER_DAY * (DATETIME_MIN_JULIAN - UNIX_EPOCH_JDATE)))
		ereport(ERROR,
			(errcode(ERRCODE_DATETIME_VALUE_OUT_OF_RANGE),
			 errmsg("timestamp out of range")));
...
#ifdef HAVE_INT64_TIMESTAMP
	timestamp =
	    microseconds - ((POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE) * USECS_PER_DAY);
#else
	...
	timestamp = (float8)((int64) seconds -
...
Datum ts_time_to_internal(PG_FUNCTION_ARGS)
{
	if (PG_ARGISNULL(0))
		PG_RETURN_NULL();

	PG_RETURN_INT64(time_value_to_internal(
	    PG_GETARG_DATUM(0), get_fn_expr_argtype(fcinfo->flinfo, 0), false));
}
...
create_fmgr(char *schema, char *function_name, int num_args)
{
	...
	FuncCandidateList func_list = FuncnameGetCandidates(
	    list_make2(makeString(schema), makeString(function_name)),
	    num_args,
	    NULL,
	    false,
	    false,
	...
	{
		elog(ERROR,
		     "could not find the function \"%s.%s\"",
		     schema,
		     function_name);
	}
...
get_interval_period_approx(Interval *interval)
{
	return interval->time +
	       (((interval->month * DAYS_PER_MONTH) + interval->day) * USECS_PER_DAY);
}
...
date_trunc_interval_period_approx(text *units)
{
	...
	char *lowunits = downcase_truncate_identifier(
	    VARDATA_ANY(units), VARSIZE_ANY_EXHDR(units), false);
...
inheritance_parent_relid(Oid relid)
{
	...
	scan =
	    systable_beginscan(catalog, InheritsRelidSeqnoIndexId, true, NULL, 1, &skey);
...
type_is_int8_binary_compatible(Oid sourcetype)
{
	...
	tuple = SearchSysCache2(
	    CASTSOURCETARGET, ObjectIdGetDatum(sourcetype), ObjectIdGetDatum(INT8OID));
	if (!HeapTupleIsValid(tuple))
