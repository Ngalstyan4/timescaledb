...
static double custom_group_estimate_time_bucket(PlannerInfo *root, FuncExpr *expr,
						double path_rows);
static double custom_group_estimate_date_trunc(PlannerInfo *root, FuncExpr *expr,
					       double path_rows);
...
typedef struct
{
	...
	char *function_name;
	int   nargs;
	double (*custom_group_estimate_func)(PlannerInfo *root, FuncExpr *expr,
					     double path_rows);
...
initialize_custom_estimate_func_info()
{
	...
	for (i = 0; i < _MAX_HASHAGG_FUNCTIONS; i++)
	{
		...
		if (def.extension_function)
			funclist = FuncnameGetCandidates(
			    list_make2(makeString(extension_schema_name),
		...
		while (!function_found && funclist != NULL)
		{
			if (funclist->nargs != def.nargs ||
				...
				funclist->args, def.arg_types, def.nargs))
...
get_custom_estimate_func_info(Oid function_oid)
{
	...
	hash_entry =
	    hash_search(custom_estimate_func_hash, &function_oid, HASH_FIND, NULL);
...
estimate_max_spread_var(PlannerInfo *root, Var *var)
{
	...
	get_sort_group_operators(
	    var->vartype, true, false, false, &ltop, NULL, NULL, NULL);
...
custom_group_estimate_time_bucket(PlannerInfo *root, FuncExpr *expr, double path_rows)
{
	...
	case INTERVALOID:
		period =
		    (double) get_interval_period_approx(DatumGetIntervalP(c->constvalue));
		break;
	default:
		return INVALID_ESTIMATE;
	}
...
custom_group_estimate_date_trunc(PlannerInfo *root, FuncExpr *expr, double path_rows)
{
	...
	return custom_group_estimate_expr_interval(
	    root, second_arg, (double) date_trunc_interval_period_approx(interval));
}

/* if performing integer division number of groups is less than the spread divided by the
 * divisor. Note that this is an overestimate. */
static double
custom_group_estimate_integer_division(PlannerInfo *root, Oid opno, Node *left,
				       Node *right)
{
	...
	if (function_name[0] == '/' && function_name[1] == '\0' && IsA(right, Const))
	{
		...
		if (c->consttype != INT2OID && c->consttype != INT4OID &&
		    c->consttype != INT8OID)
			return INVALID_ESTIMATE;

		return custom_group_estimate_expr_interval(
		    root, (Expr *) left, (double) c->constvalue);
	}
	return INVALID_ESTIMATE;
}
			       ...
			       double path_rows)
{
	...
	if (NULL != func_est)
		return func_est->custom_group_estimate_func(
		    root, custom_group_estimate_func, path_rows);
	return INVALID_ESTIMATE;
}
...
custom_group_estimate_opexpr(PlannerInfo *root, OpExpr *opexpr, double path_rows)
{
	...
	estimate =
	    custom_group_estimate_integer_division(root, opexpr->opno, first, second);
...
/* Get a custom estimate for the number of groups of an expression. Return
 * INVALID_ESTIMATE if we don't have any extra knowledge and should just use the default
 * estimate */
static double
...
/* Get a custom estimate for the number of groups in a query. Return INVALID_ESTIMATE if
 * we don't have any extra knowledge and should just use the default estimate. This works
 * by getting a custom estimate for any groups where a custom estimate exists and
 * multiplying that by the standard estimate of the groups for which custom estimates
 * don't exist */
static double
custom_group_estimate(PlannerInfo *root, double path_rows)
{
	...
	if (new_group_expr != NIL)
		d_num_groups *=
		    estimate_num_groups(root, new_group_expr, path_rows, NULL);
...
static void
plan_add_parallel_hashagg(PlannerInfo *root, RelOptInfo *input_rel,
			  RelOptInfo *output_rel, double d_num_groups)
{
	...
	double d_num_partial_groups =
	    custom_group_estimate(root, cheapest_partial_path->rows);
	...
	if (parse->hasAggs)
	{
		...
		get_agg_clause_costs(
		    root, parse->havingQual, AGGSPLIT_FINAL_DESERIAL, &agg_final_costs);
	}

	hashagg_table_size = estimate_hashagg_tablesize(
	    cheapest_partial_path, &agg_partial_costs, d_num_partial_groups);
	...
	partial_path = (Path *) create_gather_path(
	    root, output_rel, partial_path, partial_grouping_target, NULL, &total_groups);
	add_path(output_rel,
		 (Path *) create_agg_path(root,
					  output_rel,
					  partial_path,
...
void
plan_add_hashagg(PlannerInfo *root, RelOptInfo *input_rel, RelOptInfo *output_rel)
{
	Query *	parse = root->parse;
	...
	get_agg_clause_costs(
	    root, (Node *) root->processed_tlist, AGGSPLIT_SIMPLE, &agg_costs);
	...
	hashaggtablesize =
	    estimate_hashagg_tablesize(cheapest_path, &agg_costs, d_num_groups);
