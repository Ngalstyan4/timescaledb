...
create_index_colnames(Relation indexrel)
{
	...
	for (i = 0; i < indexrel->rd_att->natts; i++)
		colnames = lappend(colnames,
				   pstrdup(NameStr(indexrel->rd_att->attrs[i]->attname)));
 ...
 */
static void
chunk_adjust_expr_attnos(IndexInfo *ii, Relation htrel, Relation idxrel,
			 Relation chunkrel)
{
	ListCell *lc;
	...
	foreach (lc, ii->ii_Expressions)
	{
		...
		foreach (lc_var, vars)
		{
			...
			Name attname =
			    find_attname_by_attno(htrel->rd_att, var->varattno);

			if (NULL == attname)
				elog(ERROR,
				     "index expression var %u not found in chunk",
				     var->varattno);
			...
			if (var->varattno == InvalidAttrNumber)
				elog(ERROR,
				     "index attribute %s not found in chunk",
				     NameStr(*attname));
		}
	}
}
...
chunk_adjust_colref_attnos(IndexInfo *ii, Relation idxrel, Relation chunkrel)
{
	...
	for (i = 0; i < idxrel->rd_att->natts; i++)
	{
		...
		AttrNumber	     attno =
		    find_attno_by_attname(chunkrel->rd_att, &idxattr->attname);
...
chunk_index_select_tablespace(Relation htrel, Relation chunkrel)
{
	...
	tspc = hypertable_get_tablespace_at_offset_from(
	    ht, chunkrel->rd_rel->reltablespace, CHUNK_INDEX_TABLESPACE_OFFSET);
			    ...
			    bool isconstraint)
{
	 ...
	 * hypertable
	 */
	if (chunk_index_need_attnos_adjustment(RelationGetDescr(htrel),
					       RelationGetDescr(chunkrel)))
		chunk_adjust_attnos(indexinfo, htrel, template_indexrel, chunkrel);

	tuple = SearchSysCache1(RELOID,
				ObjectIdGetDatum(RelationGetRelid(template_indexrel)));
	...
	indexname =
	    chunk_index_choose_name(get_rel_name(RelationGetRelid(chunkrel)),
			    ...
			    int32 hypertable_id, const char *parent_index)
{
	...
	values[AttrNumberGetAttrOffset(Anum_chunk_index_chunk_id)] =
	    Int32GetDatum(chunk_id);
	values[AttrNumberGetAttrOffset(Anum_chunk_index_index_name)] =
	    DirectFunctionCall1(namein, CStringGetDatum(chunk_index));
	values[AttrNumberGetAttrOffset(Anum_chunk_index_hypertable_id)] =
		   ...
		   const char *hypertable_index)
{
	...
	result = chunk_index_insert_relation(
	    rel, chunk_id, chunk_index, hypertable_id, hypertable_index);
...
void
chunk_index_create_from_constraint(int32 hypertable_id, Oid hypertable_constraint,
 ...
 */
static void
chunk_index_create(Relation hypertable_rel, int32 hypertable_id,
		   ...
		   Oid constraint_oid)
{
	...
	chunk_indexrelid = chunk_relation_index_create(
	    hypertable_rel, hypertable_idxrel, chunkrel, false);
 ...
 */
Oid
chunk_index_create_from_stmt(IndexStmt *stmt, int32 chunk_id, Oid chunkrelid,
			     int32 hypertable_id, Oid hypertable_indexrelid)
{
	...
	chunk_index_insert(
	    chunk_id, get_rel_name(idxobj.objectId), hypertable_id, hypertable_indexname);
 ...
 */
void
chunk_index_create_all(int32 hypertable_id, Oid hypertable_relid, int32 chunk_id,
		       Oid chunkrelid)
{
	Relation  htrel;
	Relation  chunkrel;
	...
	foreach (lc, indexlist)
	{
		...
		Relation hypertable_idxrel =
		    relation_open(hypertable_idxoid, AccessShareLock);
...
chunk_index_scan(int indexid, ScanKeyData scankey[], int nkeys,
		 tuple_found_func tuple_found, tuple_filter_func tuple_filter, void *data,
		 LOCKMODE lockmode)
{
	Catalog *  catalog = catalog_get();
	ScannerCtx scanCtx = {
...
#define chunk_index_scan_update(idxid, scankey, nkeys, tuple_found, tuple_filter, data)  \
	chunk_index_scan(                                                                \
	    idxid, scankey, nkeys, tuple_found, tuple_filter, data, RowExclusiveLock)
...
chunk_index_tuple_delete(TupleInfo *ti, void *data)
{
	...
	if (cid->drop_index)
	{
		ObjectAddress idxobj = {
			...
			.objectId =
			    get_relname_relid(NameStr(chunk_index->index_name), schemaid),
		};
			       ...
			       bool should_drop)
{
	...
	return chunk_index_scan_update(
...
chunk_index_tuple_rename(TupleInfo *ti, void *data)
{
	...
	if (info->isparent)
	{
		...
		Chunk *chunk = chunk_get_by_id(chunk_index->chunk_id, 0, true);
		Oid    chunk_schemaoid =
		    get_namespace_oid(NameStr(chunk->fd.schema_name), false);
		const char *chunk_index_name = chunk_index_choose_name(
		    NameStr(chunk->fd.table_name), info->newname, chunk_schemaoid);
		Oid chunk_indexrelid =
		    get_relname_relid(NameStr(chunk_index->index_name), chunk_schemaoid);
...
int
chunk_index_set_tablespace(Hypertable *ht, Oid hypertable_indexrelid,
...
Datum ts_chunk_index_clone(PG_FUNCTION_ARGS)
{
	...
	new_chunk_indexrelid = chunk_relation_index_create(
	    hypertable_rel, chunk_index_rel, chunk_rel, OidIsValid(constraint_oid));
...
Datum ts_chunk_index_replace(PG_FUNCTION_ARGS)
{
