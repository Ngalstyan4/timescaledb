...
hypertable_from_tuple(HeapTuple tuple, MemoryContext mctx)
{
	...
	h->space =
	    dimension_scan(h->fd.id, h->main_table_relid, h->fd.num_dimensions, mctx);
	h->chunk_cache =
	    subspace_store_init(h->space, mctx, guc_max_cached_chunks_per_hypertable);
	...
	{
		FuncCandidateList func = FuncnameGetCandidates(
		    list_make2(makeString(NameStr(h->fd.chunk_sizing_func_schema)),
...
{
	Catalog *  catalog = catalog_get();
	ScannerCtx scanctx = { .table = catalog->tables[HYPERTABLE].id,
...
hypertable_tuple_update(TupleInfo *ti, void *data)
{
	...
	values[AttrNumberGetAttrOffset(Anum_hypertable_table_name)] =
	...
	{
		Dimension *dim =
		    hyperspace_get_dimension(ht->space, DIMENSION_TYPE_OPEN, 0);
	...
	{
		nulls[AttrNumberGetAttrOffset(Anum_hypertable_chunk_sizing_func_schema)] =
		    true;
		nulls[AttrNumberGetAttrOffset(Anum_hypertable_chunk_sizing_func_name)] =
		    true;
	}
...
int
hypertable_scan_with_memory_context(const char *schema, const char *table,
				    tuple_found_func tuple_found, void *data,
				    LOCKMODE lockmode, bool tuplock, MemoryContext mctx)
{
...
int
hypertable_scan_relid(Oid table_relid, tuple_found_func tuple_found, void *data,
...
hypertable_tuple_delete(TupleInfo *ti, void *data)
{
	...
	bool		       isnull;
	int		       hypertable_id =
	    heap_getattr(ti->tuple, Anum_hypertable_id, ti->desc, &isnull);
	...
	case HeapTupleUpdated:
		ereport(ERROR,
			(errcode(ERRCODE_LOCK_NOT_AVAILABLE),
			 errmsg("hypertable \"%s\" has already been updated by another "
				"transaction",
				get_rel_name(table_relid)),
			   ...
			   int64 chunk_target_size, int16 num_dimensions)
{
	...
	values[AttrNumberGetAttrOffset(Anum_hypertable_id)] =
	    Int32GetDatum(catalog_table_next_seq_id(catalog_get(), HYPERTABLE));
	...
	else
	{
		...
		snprintf(
		    NameStr(default_associated_table_prefix),
		    NAMEDATALEN,
...
hypertable_insert(Name schema_name, Name table_name, Name associated_schema_name,
		  ...
		  Name chunk_sizing_func_name, int64 chunk_target_size,
...
hypertable_get_by_name(char *schema, char *name)
{
	...
	hypertable_scan(
	    schema, name, hypertable_tuple_found, &ht, AccessShareLock, false);
...
hypertable_check_associated_schema_permissions(const char *schema_name, Oid user_oid)
{
	...
	if (!OidIsValid(schema_oid))
	{
		 ...
		 */
		if (pg_database_aclcheck(MyDatabaseId, user_oid, ACL_CREATE) !=
		    ACLCHECK_OK)
			ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permissions denied: cannot create schema \"%s\" "
					"in database \"%s\"",
					schema_name,
...
hypertable_validate_constraints(Oid relid)
{
	...
	scan =
	    systable_beginscan(catalog, ConstraintRelidIndexId, true, NULL, 1, &scankey);
		...
		if (form->contype == CONSTRAINT_CHECK && form->connoinherit)
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
				 errmsg("cannot have NO INHERIT constraints on "
					"hypertable \"%s\"",
					get_rel_name(relid)),
				 errhint("Remove all NO INHERIT constraints from table "
					 "\"%s\" before making it a hypertable.",
					 get_rel_name(relid))));
	}
	...
	if (guc_restoring)
		ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot INSERT into hypertable \"%s\" during restore",
				relname),
			 errhint("Set 'timescaledb.restoring' to 'off' after the restore "
				 "process has finished.")));
	else
		ereport(
		    ERROR,
		    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		     errmsg("invalid INSERT on the root table of hypertable \"%s\"",
			    relname),
...
insert_blocker_trigger_add(Oid relid)
{
	...
	CreateTrigStmt stmt = {
		...
		.funcname = list_make2(makeString(INTERNAL_SCHEMA_NAME),
				       makeString(OLD_INSERT_BLOCKER_NAME)),
		.args = NIL,
	 ...
	 */
	objaddr =
	    CreateTrigger(&stmt, NULL, relid, InvalidOid, InvalidOid, InvalidOid, false);
...
Datum ts_hypertable_insert_blocker_trigger_add(PG_FUNCTION_ARGS)
{
	...
	if (table_has_tuples(relid, AccessShareLock))
		ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("hypertable \"%s\" has data in the root table",
				get_rel_name(relid)),
			 errdetail("Migrate the data from the root table to chunks "
	...
	{
		ObjectAddress objaddr = { .classId = TriggerRelationId,
...
create_hypertable_datum(FunctionCallInfo fcinfo, Hypertable *ht, bool created)
{
	...
	values[AttrNumberGetAttrOffset(Anum_create_hypertable_id)] =
	    Int32GetDatum(ht->fd.id);
	...
	values[AttrNumberGetAttrOffset(Anum_create_hypertable_table_name)] =
	    NameGetDatum(&ht->fd.table_name);
	values[AttrNumberGetAttrOffset(Anum_create_hypertable_created)] =
...
Datum ts_hypertable_create(PG_FUNCTION_ARGS)
{
	...
	case RELKIND_PARTITIONED_TABLE:
		ereport(ERROR,
			(errcode(ERRCODE_WRONG_OBJECT_TYPE),
			 errmsg("table \"%s\" is already partitioned",
				get_rel_name(table_relid)),
			 errdetail("It is not possible to turn partitioned tables into "
				   "hypertables.")));
#endif
	...
	if (!migrate_data && table_has_data)
		ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 ...
			 errhint("You can migrate data by specifying 'migrate_data => "
	...
	if (is_inheritance_table(table_relid))
		ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("table \"%s\" is already partitioned",
				get_rel_name(table_relid)),
			 errdetail("It is not possible to turn tables that use "
				   "inheritance into hypertables.")));
	...
	if (!table_is_logged(table_relid))
		ereport(
		    ...
		    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		     ...
		     errdetail("It is not possible to turn temporary or unlogged tables "
			       "into hypertables.")));
	...
	if (table_has_replica_identity(rel))
		ereport(
		    ERROR,
		    (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		     errmsg("table \"%s\" has replica identity set",
			    get_rel_name(table_relid)),
	...
	if (table_has_rules(rel))
		ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 ...
			 errdetail("Table \"%s\" has attached rules, which do not work "
				   "on hypertables.",
				   get_rel_name(table_relid)),
	...
	associated_schema_oid = hypertable_check_associated_schema_permissions(
	    NameStr(*associated_schema_name), user_oid);
	...
	hcache = hypertable_cache_pin();
	time_dim_info.ht = space_dim_info.ht =
	    hypertable_cache_get_entry(hcache, table_relid);
	...
	if (table_has_data)
	{
		ereport(NOTICE,
			(errmsg("migrating data to chunks"),
			 errdetail("Migration might take a while depending on the amount "
				   "of data.")));
