...
execute_drop_chunks_policy(int32 job_id)
{
	...
	else
	{
		...
		Assert(IS_INTEGER_TYPE(ts_dimension_get_partition_type(open_dim)));
		...
		older_than_datum =
			ts_integer_from_now_func_get_datum(args->fd.older_than_integer,
											   ts_dimension_get_partition_type(open_dim),
											   now_func);

		/* When getting older_than_datum, we do another validation and only proceed if now_func
		 * return type is the same as time_dim_type that is why it is safe to pass partitioning type
		 * in place of older_than_datum type.
		 */
		ts_chunk_do_drop_chunks(table_relid,
								older_than_datum,
								(Datum) 0,
