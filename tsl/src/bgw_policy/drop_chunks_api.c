...
ts_integer_from_now_func_get_datum(int64 interval, Oid time_dim_type, Oid now_func) {
	...
	// bgw luncher starts a transactions so here I can assume I am always inside a transaction right?
	// no need for StartTransactionCommand();
...
void ts_integer_now_func_validate(Oid now_func_oid, Oid open_dim_type) {
	HeapTuple tuple;
	...
	switch (older_than_type) {
		case INTERVALOID:
			if (IS_INTEGER_TYPE(partitioning_type))
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
							errmsg("interval older_than argument can only be used with hypertables "
							"that have non-integer time dimensions")));// <<-- time columns? open dimensions? TODO::
		...
		case INT8OID:
			...
			/* TODO:: q:: awkward way to check if a NameStr col is null or not but did not find a better API */
